...
`demo/plugin.js` holds six GLSL literals and so does `source/shaders/`. That is
...
# Two of these are not whole shaders. The disperse pass is assembled at runtime
# from kFieldFunctions + kDisperseCommon + kDisperseMain behind a header carrying
# MAX_SAMPLES, in both the plugin and the page -- so the pieces are what there is
# to compare, and comparing them is strictly better than comparing the assembled
# result, because a difference points at a file instead of at a concatenation.
#
# kDisperseCommon and kDisperseMain both live in Disperse.cpp, inside an
# anonymous namespace. The regex matches on the symbol, not the file, so that is
# not a problem -- but it does mean a symbol renamed in the C++ shows up here as
# "not found" rather than as drift.
SHADERS = [
    ("VERTEX", "source/shaders/Vertex.cpp", "kVertex"),
    ("COPY", "source/shaders/Copy.cpp", "kCopyFragment"),
    ("EDGE", "source/shaders/Edge.cpp", "kEdgeFragment"),
    ("FIELD_FUNCTIONS", "source/shaders/Field.cpp", "kFieldFunctions"),
    ("DISPERSE_COMMON", "source/shaders/Disperse.cpp", "kDisperseCommon"),
    ("DISPERSE_MAIN", "source/shaders/Disperse.cpp", "kDisperseMain"),
]
...
def from_cpp(path, symbol):
    with open(os.path.join(REPO, path)) as handle:
        source = handle.read()
    match = re.search(r'const char\* const ' + symbol + r'\s*=\s*R"\((.*?)\)";', source, re.S)
    if match is None:
        return None
...
def main():
    ...
    for name, path, symbol in SHADERS:
        ...
        if cpp_text == js_text:
            print(f"ok    {name:<15} matches {path} ({len(cpp_text)} chars)")
            continue
